...
def ShowTravelBookDelayed():
    import Menu
    if (not Menu.MENU_PREACTIVATED):
        time = Bladex.GetTime()
        Bladex.AddScheduledFunc(time,ExecTravelBookNP,())
...
def InitBindings (PlayerName):
	...
	Bladex.AddBoundFunc("RotateOrMoveX","RotateOrMoveX")
	Bladex.AddBoundFunc("RotateOrMoveY","RotateOrMoveY")
...
ConfigurableActions =[
			...
			(TranslateBoth("Use / Special 2"),      "UseAndLeftRight",               []),
...
def removeAction(action):
	for i in range(0,len(ConfigurableActions)-1):
		if ConfigurableActions[i][0] == action:
			del ConfigurableActions[i]

if Bladex.IsConsole():
	removeAction("Screen Shot")
	removeAction("Look Up")
	removeAction("Look Down")
	removeAction("Free Look")
