...
def GetActivityMapName(value):
	if string.lower(value) in ['barb_m1','ragnar_m2','dwarf_m3','ruins_m4']:
		return 'FIRST_LEVEL'
	else:
		return string.upper(value)


def GetVisitedMapNames():
	value = MemPersistence.Get('VisitedMapNames')
	if value is None:
		value = []
	return value

def SetVisitedMapNames(value):
	if value is None:
		value = []

	MemPersistence.Store("VisitedMapNames",value)

	current = GetActivityMapName(Bladex.GetCurrentMap())

	visited = []

	for name in value:
		final_name = GetActivityMapName(name)
		if final_name not in visited:
			visited.append(final_name)

	Bladex.LoadMapFromSave(current, visited)


def BeginLevel():
	print "BeginLevel"

	Bladex.LevelStarted( string.upper(Bladex.GetCurrentMap() ) )
...
def StoreCharInfo():
	...
	try:
		...
		visitedMapNames = GetVisitedMapNames()
		visitedMapNames.append(name)
		...
		MemPersistence.Store('VisitedMapNames', visitedMapNames)
...
def EndOfLevel():
	...
	import Language
	import SplashImage
	scr_name="../../Data/Menu/Save/Cargando_hi_empty.jpg"
	...
	Bladex.LevelEnded(GetActivityMapName(Bladex.GetCurrentMap()))
