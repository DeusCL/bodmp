...
class Dummy:
  def __init__(self):
    r = whrandom.randint(1, 6)
    self.MenuDescr={"Clave": str(r)}
...
def DebugChangeMap():
  ...
  name = names[whrandom.randint(0, len(names) -1)]
  Bladex.LoadLevel(name)

def DebugSaveLoadGameAndChangeMap():
  descr = Dummy()
  r = whrandom.randint(0, 2)

  Bladex.AddScheduledFunc(Bladex.GetTime() + 2, DebugSaveLoadGameAndChangeMap,())

  if r == 0:
    SaveGame.SaveGameToDisk(descr)
  elif r == 1:
    SaveGame.LoadGameFromDisk(descr)
  elif r == 2:
    DebugChangeMap()


Bladex.AddInputAction("Test", 0)
Bladex.AssocKey("Test", "Keyboard", "L")
Bladex.AddBoundFunc("Test", DebugSaveLoadGameAndChangeMap)

print "End of DebugControl.py"
