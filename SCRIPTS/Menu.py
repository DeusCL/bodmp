...
if Bladex.IsRunningOnSteamDeck():
    MenuFontMed=Language.LetrasMenuBig

Bladex.SetDefaultFont(MenuFontBig)
...
languages = Language.ExistingLanguages
languages.sort()
languagesTranslated = Language.GetTranslatedLanguages()

selected_language_index = -1

LockedMenu = 0

AcceptBack = "${ignore:ButtonActionValidate}${ignore:\" - " + MenuText.GetMenuText("Accept") + " / }${ignore:ButtonActionBack}${ignore:\" - " + MenuText.GetMenuText("Back") + "\"}"
...
def GetLanguage():
  try:
    return languagesTranslated.index(MenuText.GetMenuText(Language.Current))
  except:
    return 0
...
def SetLanguage(option):
  ...
  try:
    selected_language_index = languages.index(option)
  except:
...
def ResetKeyB(option):
    ...
    Bladex.SetFieldOfView(114)
    Bladex.SetReworkedCamera(1) # Reworked
...
def ReworkedCamera():
  opt_value = Bladex.GetReworkedCamera();
  if opt_value == 2: # Reworked
    return 2
  elif opt_value == 1: # Intermediate
    return 1
  else: # Original
    return 0
...
def SetVerticalSyncValue(option, target):
  enabled = (option == "Yes")
  Bladex.SetVerticalSync(enabled)
  target.Parent.MenuItems[4].focusable = not enabled
  return

def GetLimitFps():
  return Bladex.GetLimitFps()

def SetLimitFps(option, target):
  enabled = (option == "Yes")
  Bladex.SetLimitFps(enabled)
  target.Parent.MenuItems[3].focusable = not enabled
  return

def FocusableWhenNoLimitFps(target):
  target.focusable = not Bladex.GetLimitFps()
  return 0

def FocusableWhenNoVerticalSync(target):
  target.focusable = not Bladex.GetVerticalSync()
  return 0
...
def GetEdgeAntialiasing():
  return Bladex.GetEdgeAntialiasing()

def SetEdgeAntialiasing(option):
  Bladex.SetEdgeAntialiasing(option=="Yes")
  return 0

def GetSsao():
  return Bladex.GetSsao()

def SetSsao(option):
  Bladex.SetSsao(option=="Yes")
  return 0

def GetMotionBlur():
  return Bladex.GetMotionBlur()

def SetMotionBlur(option):
  Bladex.SetMotionBlur(option=="Yes")
  return 0

def GetHdr():
  return Bladex.GetHdr()

def SetHdr(option, target):
  Bladex.SetHdr(option=="Yes")
  bloomId = 9
  if Bladex.IsConsole():
    bloomId = 5
  FocusableWhenHdrEnable(target.Parent.MenuItems[bloomId])
  return 0

def GetBloom():
  return Bladex.GetBloom()

def SetBloom(option):
  Bladex.SetBloom(option=="Yes")
  return 0

def FocusableWhenHdrEnable(target):
  target.focusable = Bladex.GetHdr()
  return 0
...
def GetSpeakerConfig():
    speakerId = Bladex.GetSpeakerConfig()

    if Bladex.IsRunningOnPlayStation() == 1:
      if speakerId > 1:
        return 2 #:NOTE:7.1 
...
def ActivateMenu(caller_id = None):
  ...
  if AppMode=="Game" or AppMode=="Demo":
    ...
    Bladex.AssocKey("Menu ActivateItem","Gamepad","ButtonActionValidate")
    ...
    Bladex.AssocKey("Menu DeActivateItem","Gamepad","ButtonActionBack")
...
def ClearMenuKeyb():
  InputManager=BInput.GetInputManager()
  current = InputManager.GetInputActionsSet()
  InputManager.SetInputActionsSet("Menu")
  ...
  InputManager.SetInputActionsSet(current)
  global LockedMenu
  LockedMenu = 0
...
#Raster.LoadBitmap("Background","../../Data/Menu.bmp")
  ...
  def DeActivateMenuItem(self):
    print "DeActivateMenuItem"
    global EscapeFunction
    if Bladex.GetAppMode()=="Menu":
      Bladex.SetGamepadChangeFunc()
      if (self.MenuStack.nItems() == 1):
        if (EscapeFunction!=None):
          if(EscapeFunction(0)):
            return
        else:
          InputManager=BInput.GetInputManager()
      ...
      try:
        ...
        try:
          ...
          print("Exception in DeActivateMenuItem")
      #print "Deact"
      ...
      if selected_language_index != -1:
        new_lang = languages[selected_language_index]
        if new_lang != Language.Current:
           BBLib.SetLanguage(new_lang)
           Bladex.LoadLevel("Casa")
...
def AuxCtrlDef(action_menu_name,action_name,kFlags):
    ...
    return res
KeybActions=[]
actionToRemove = ["Use", "Jump", "Sneak", "Select Object", "Free Look", "Next View", "Look Up", "Look Down"]
for k in acts.ConfigurableActions:
  if Bladex.IsConsole():
    if k[0] in actionToRemove:
      continue
  KeybActions.append(AuxCtrlDef(k[0],k[1],k[2]))

def AuxAchDef(achievement_data):
    res = {"Name":achievement_data[0],
             "Position" :CtrlsPosition,
             "Kind"     : AchievementsMenu.AchievementItem,
             "kFlags"   : 1,
             "VSep" : 34,
             "Size" :(200,300),
             "Description":achievement_data[1],
             "Id":achievement_data[2],
            }
    return res
#LogoItem={"Name":"Logo",
#          "VSep":50,
...
#                                                         }
                                           ...
                                           {"Name":MenuText.GetMenuText("CAMERA"),
                                            ...
                                            "ListDescr":[
                                                         {"Name":MenuText.GetMenuText("Reworked Camera:"),
                                                          ...
                                                          "Font":MenuFontMed,
                                                          "Kind":MenuWidget.B_MenuItemOption,
                                                          "Options":[MenuText.GetMenuText("0"),MenuText.GetMenuText("1"),MenuText.GetMenuText("2")],
                                                          "SelOptionFunc":ReworkedCamera,
                                                          "Command":SetReworkedCamera,
                                                          "Size": (600, 20)
                                                         },
                                                         {"Name":MenuText.GetMenuText("Invert") + " X",
                                                          "Font":MenuFontMed,
                                                          "Kind":MenuWidget.B_MenuItemOption,
                                                         ...
                                                         {"Name":MenuText.GetMenuText("X Speed"),
                                                          "Font":MenuFontMed,
                                                          "Kind":MenuWidget.B_MenuSpin,
                                                          "SpinValues":(int(0),int(100),int(100)),
                                           ...
                                           {"Name":MenuText.GetMenuText("GAMEPAD"),
                                            "Font":MenuFontBig,
                                            "Size":(640,480),
                                            "ListDescr":[{"Name":MenuText.GetMenuText("Gamepad settings"),
...
Description_MotionBlur = {
  "Name":MenuText.GetMenuText("MOTION_BLUR:"),
  "VSep": 100,
  "Font":MenuFontMed,
  "Kind":MenuWidget.B_MenuItemOption,
  "Options":[MenuText.GetMenuText("No"),MenuText.GetMenuText("Yes")],
  "SelOptionFunc":GetMotionBlur,
  "Command":SetMotionBlur,
}

Description_Hdr = {
  "Name":MenuText.GetMenuText("HDR Tone-mapping:"),
  "VSep": 15,
  "Font":MenuFontMed,
  "Kind":MenuWidget.B_MenuItemOption,
  "Options":[MenuText.GetMenuText("No"),MenuText.GetMenuText("Yes")],
  "SelOptionFunc":GetHdr,
  "Command2":SetHdr,
}

Description_Bloom = {
  "Name":MenuText.GetMenuText("BLOOM:"),
  "VSep": 15,
  "Font":MenuFontMed,
  "Kind":MenuWidget.B_MenuItemOption,
  "Options":[MenuText.GetMenuText("No"),MenuText.GetMenuText("Yes")],
  "SelOptionFunc":GetBloom,
  "Command":SetBloom,
  "PostInitCommand":FocusableWhenHdrEnable,
}
...
#                                                   },
                                                   {"Name":MenuText.GetMenuText("Speaker Configuration:"),
                                                    "VSep":10,
                                                    "Font":MenuFontMed,
                                                    "Options":[MenuText.GetMenuText("Mono"),MenuText.GetMenuText("2 Speakers"),MenuText.GetMenuText("7.1")],
                                                    "Kind":MenuWidget.B_MenuItemOption,
...
if "Casa" == Bladex.GetCurrentMap():
  Options_Menu["ListDescr"].insert(7,
			   {"Name":MenuText.GetMenuText("LANGUAGE:"),
					  "VSep":2,
					  "Font":MenuFontBig,
					  "Kind":MenuWidget.B_MenuItemOption,
					  "Options":languagesTranslated,
					  "Command": SetLanguage,
					  "SelOptionFunc":GetLanguage
   })

if(Bladex.IsRunningOnHandheldDevice() == 0 and Bladex.IsConsole() == 0):

    Options_Menu["ListDescr"][0]["ListDescr"][0]["VSep"] = 15

    Options_Menu["ListDescr"][0]["ListDescr"].insert(0,{"Name":MenuText.GetMenuText("SSAO:"),
                                                        "VSep": 15,
                                                        "Font":MenuFontMed,
                                                        "Kind":MenuWidget.B_MenuItemOption,
                                                        "Options":[MenuText.GetMenuText("No"),MenuText.GetMenuText("Yes")],
                                                        "SelOptionFunc":GetSsao,
                                                        "Command":SetSsao,
                                                       })

    Options_Menu["ListDescr"][0]["ListDescr"].insert(0,{"Name":MenuText.GetMenuText("Anti-Aliasing:"),
                                                        "VSep": 15,
                                                        "Font":MenuFontMed,
                                                        "Kind":MenuWidget.B_MenuItemOption,
                                                        "Options":[MenuText.GetMenuText("No"),MenuText.GetMenuText("Yes")],
                                                        "SelOptionFunc":GetEdgeAntialiasing,
                                                        "Command":SetEdgeAntialiasing,
                                                       })

    Options_Menu["ListDescr"][0]["ListDescr"].insert(0,{"Name":MenuText.GetMenuText("Framerate Limit:"),
                                                        "VSep":15,
                                                        "Font":MenuFontMed,
                                                        "Kind":MenuWidget.B_MenuItemOption,
                                                        "Options":[MenuText.GetMenuText("No"),MenuText.GetMenuText("Yes")],
                                                        "SelOptionFunc":GetLimitFps,
                                                        "Command2":SetLimitFps,
                                                        "PostInitCommand": FocusableWhenNoVerticalSync,
                                                       })

    Options_Menu["ListDescr"][0]["ListDescr"].insert(0,{"Name":MenuText.GetMenuText("Vertical Sync:"),
                                                        "VSep":15,
                                                        "Font":MenuFontMed,
                                                        "Kind":MenuWidget.B_MenuItemOption,
                                                        "Options":[MenuText.GetMenuText("No"),MenuText.GetMenuText("Yes")],
                                                        "SelOptionFunc":GetVerticalSync,
                                                        "Command2":SetVerticalSyncValue,
                                                        "PostInitCommand": FocusableWhenNoLimitFps,
                                                       })
    ...
    Options_Menu["ListDescr"][4]["ListDescr"].insert(0, GetPasswordMenu())
else:
    Options_Menu["ListDescr"][4]["ListDescr"][0]["VSep"] = 100
...
Controler_Menu["ListDescr"][1]["ListDescr"].insert(
   pos,
...
if Bladex.IsRunningOnPlayStation() == 0:
  soundOptions = Options_Menu["ListDescr"][3]["ListDescr"][2]["Options"]
  soundOptions.insert(2, MenuText.GetMenuText("4 Speakers"))
  soundOptions.insert(3, MenuText.GetMenuText("5.1"))

if Bladex.IsConsole():
    del Options_Menu["ListDescr"][0]
    Options_Menu["ListDescr"][0]["VSep"] = 100
    del Description_MotionBlur["VSep"]
    Options_Menu["ListDescr"][0]["ListDescr"].insert(3, Description_MotionBlur)
    Options_Menu["ListDescr"][0]["ListDescr"].insert(4, Description_Hdr)
    Options_Menu["ListDescr"][0]["ListDescr"].insert(5, Description_Bloom)
...
if netgame.GetNetState() != 0:
  ...
  DevMenu.Init(Desc1["ListDescr"])

  if Bladex.IsConsole():
    del Desc1["ListDescr"][Desc1["ListDescr"].index(QuitMenu)]

  if(Bladex.IsGlobalServicesActive()):
    Achievements=[]
    for i in range(56):
      data = Bladex.GetAchievementData(i, 0)
      Achievements.append(AuxAchDef(data))

    Achievement_Menu = {"Name":MenuText.GetMenuText("ACHIEVEMENTS"),
                    "VSep":8,
                    "Font":MenuFontBig,
                    "Size":(640,480),
                    "ListDescr":[{"Name":"AchievementsMenu",
                                                    "Kind":AchievementsMenu.AchievementsMenuWidget,
                                                    "VSep":100,
                                                    "Size":(850, 500),
                                                    "ListDescr":[],
    ...
    Desc1["ListDescr"].insert(3,Achievement_Menu)
  ########## RODRIGO!! AQUI ESTA COMO SE HACE
  BackToGame = {"Name":MenuText.GetMenuText("BACK TO GAME"),
                                            "VSep":8,
                                            "Font":MenuFontBig,
                                            "Command":BackMenu,
                                           }
  if "Casa" != Bladex.GetCurrentMap() and "final" != string.lower(Bladex.GetCurrentMap()):
    Desc1["ListDescr"].insert(1,BackToGame)
    ########## ENJOY!
...
def ChangeInputActionsSetImpl(name):
    InputManager=BInput.GetInputManager()
    InputManager.SetInputActionsSet(name)
    global LockedMenu
    LockedMenu = 0



def ChangeInputActionsSet(name):
    print "ChangeInputActionsSet"
    print name
    global LockedMenu
    LockedMenu = 1
    Bladex.AddScheduledFunc(Bladex.GetTime(), ChangeInputActionsSetImpl, (name,))
