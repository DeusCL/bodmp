...
##		GameStateAux.PersistentObject.__init__(self)
	...
	def __setstate__(self,parm):
		...
		if parm[0]==1:
			...
			if len(parm) >= 43:
				self.start_pos = parm[39]
				self.start_time = parm[40]
				self.displ_time = parm[41]
				self.maxDisp = parm[42]
			else:
				# Following values are not correct but avoid crashes:
				if self.obj != None:
					self.start_pos = self.obj.Position
