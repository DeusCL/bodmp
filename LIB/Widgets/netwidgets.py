...
class B_ImageListWidget(BUIx.B_RectWidget):
	...
	def Draw(self,x,y,time):
		...
		if self.MaxHeight is None:
			Raster.DrawImage(self.vidw,self.vidh, "RGB","Normal",self.Bitmap[self.IDX]);
		else:
			NewWidth = (float(self.vidw)/float(self.vidh))*self.MaxHeight
			Raster.SetPosition(x+(self.vidw - NewWidth)/2,y)
			Raster.DrawResizeImage(self.vidw,self.vidh, "RGB","Normal",self.Bitmap[self.IDX], NewWidth, self.MaxHeight)
...
class B_MapListWidget(BUIx.B_RectWidget,MenuWidget.B_MenuTreeItem):
	...
	def Draw(self,x,y,time):
		...
		if self.Bitmap[first][3]:
			...
			Raster.DrawImage(self.SSize,self.vidh,"BGR","Normal",self.MarkMap)
		...
		if self.GetHasFocus():
			...
			Raster.DrawImage(self.LSize,self.vidh-2*self.MapH,"BGR","Normal",self.LeftMap)

			Raster.SetPosition(x+self.SSize*3+self.LSize, y+self.MapH)
			Raster.DrawImage(self.LSize,self.vidh-2*self.MapH,"BGR","Normal",self.RightMap)

			if int(time*5)%2 == 0:
				Raster.SetPosition(x+self.SSize*1+self.LSize, y)
				Raster.DrawImage(self.SSize,self.vidh,"BGR","Normal",self.SelMap)


		Raster.SetPosition(x+self.MapW+self.SSize*0+self.LSize, y+self.MapH)
		Raster.DrawImage(self.SSize-2*self.MapW,self.vidh-2*self.MapH,"BGR","Normal",self.Bitmap[first][0])


		Raster.SetPosition(x+self.MapW+self.SSize*1+self.LSize, y+self.MapH)
		Raster.DrawImage(self.SSize-2*self.MapW,self.vidh-2*self.MapH,"BGR","Normal",self.Bitmap[self.IDX][0])

		Raster.SetPosition(x+self.MapW+self.SSize*2+self.LSize, y+self.MapH)
		Raster.DrawImage(self.SSize-2*self.MapW,self.vidh-2*self.MapH,"BGR","Normal",self.Bitmap[last][0])
