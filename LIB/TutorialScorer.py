...
def LoadTBSamples(yoffset=0.42):
	global wTraveBookBmp1
	...
	if not wTraveBookBmp1:
		...
		Scorer.wFrame.AddWidget(wTraveBookBmp1,0.5,yoffset,
		...
		Scorer.wFrame.MoveWidgetTo("TravelZero",0.5,yoffset);
	...
	if not wTraveBookBmp2:
		...
		Scorer.wFrame.AddWidget(
					wTraveBookBmp2,0.5,yoffset,
					BUIx.B_Widget.B_FR_HRelative, BUIx.B_Widget.B_FR_HCenter,
					BUIx.B_Widget.B_FR_VRelative, BUIx.B_Widget.B_FR_VCenter
					)
		wTraveBookBmp2.SetAutoScale(1)
	else:
		Scorer.wFrame.MoveWidgetTo("TravelNone",0.5,yoffset);

	if Bladex.IsRunningOnSwitch() and Language.Current == "Chinese":
		wTraveBookBmp1.SetScale(0.9)
		wTraveBookBmp2.SetScale(0.9)
...
def ParseText(TextIdx):
	txt = Localization.GetTranslation(GameText.Textos, TextIdx)
	res = []
	for ln in txt:
		start = 0
		while 1:
