...
posY = 80
if Language.Current == "French":
    posY = 95
wFramePress=BUIx.B_FrameWidget(wFrame,"PressFrame",posY,18)
...
TBookSword=BUIx.B_BitmapWidget(wFrame,"TBookSword",126,22,"PRESSF1","../../Data/PRESSF1_hi_empty.png")
...
def SlideTBS(dir,time = 0):
	if Bladex.GetCurrentMap() == "Tutorial":
		return
	TBS.PlayStereo()
	SlideTBS2(dir,time)
...
def SetVisible(vis):
  ...
  if vis==0:
    ...
    import MenuWidget
    if MenuWidget.GamepadDisconnectMessageToDisplay:
      GamepadDisconnectMessageToDisplay = 0
      MenuWidget.SetGamepadDisconnectFunc(1)
    PowDefWidgets.Activate()
