...
class WorldState:
    ...
    def SaveLState(self,filename):
        ...
        aux_dir=os.path.dirname(filename)
        ...
        import LoadBar
        load_bar=LoadBar.AutoProgressBar(26,"Saving ","../../Data/Menu/Save/Guardando_hi_empty.jpg")
        load_bar.SkipFrames = 1
...
##        self.SaveFunctions(temp_dir)
        try:
            ...
            print "Could not open"+("%s/%saux"%(aux_dir,"aux"),"wt")
            return 0
...
##        file.write('InNewMap=1\n')
        ...
        #can return false
        if(Bladex.SaveSoundDataBase('%s/Sounds.sdb'%(aux_dir,))==0):
            print "Didn't save "+('%s/Sounds.sdb'%(aux_dir,))
        else:
            print "Saved "+('%s/Sounds.sdb'%(aux_dir,))
        #file.write('else:\n')
...
##        self.EntitiesState.SaveStatePass1(file,temp_dir)
        ...
        musicStateName = '%s/MusState.sdb'%(aux_dir,);
        ...
        if(SavePickledObjects(file,aux_dir)==0):
        ...
        if(self.SaveCompVars(file,aux_dir)==0):
            print "SaveCompVars failed" + aux_dir
            file.close()
            return 0
        ...
        self.SaveModulesToBeSaved(file,aux_dir,aux_dir,ModulesToBeSaved)
        ...
        file.write('GotoMapVars.SetVisitedMapNames(%s)\n'%(str(MemPersistence.Get("VisitedMapNames")),))
...
##            RM.SaveResource(B_CID_AUTO_OBJDSCR,i,name)
    ...
    def GetBMPFiles(self):
        return self.__AuxGetResFilesAndNames(BBLib.B_CID_IMAGEFILE)
