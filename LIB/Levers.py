...
def MoveLeverFunc(lever_name,time):
	...
	if lever.state==LEVER_OFF or lever.state==LEVER_ON:
		return
...
##			GameStateAux.PersistentObject.__setstate__(self,parm)
			...
			if len(parm) == 14:
				self.initialPosition = parm[12]
				self.initialOrientation = parm[13]
				c=self.type.center
				e=self.type.axis
				if self.state == LEVER_ON2OFF:
					abs_offset=self.obj.Rel2AbsVector(self.type.stick_offset[0],self.type.stick_offset[1],self.type.stick_offset[2])
					self.stick_obj.SetPosition(abs_offset[0] + self.initialPosition[0],abs_offset[1]+self.initialPosition[1],abs_offset[2]+self.initialPosition[2])
					self.stick_obj.Orientation = self.initialOrientation
					self.stick_obj.RotateRel(c[0],c[1],c[2],e[0],e[1],e[2],0)
					self.state = LEVER_OFF
					self.stick_obj.TimerFunc=None

				if self.state == LEVER_OFF2ON:
					abs_offset=self.obj.Rel2AbsVector(self.type.stick_offset[0],self.type.stick_offset[1],self.type.stick_offset[2])
					self.stick_obj.SetPosition(abs_offset[0] + self.initialPosition[0],abs_offset[1]+self.initialPosition[1],abs_offset[2]+self.initialPosition[2])
					self.stick_obj.Orientation = self.initialOrientation
					self.stick_obj.RotateRel(c[0],c[1],c[2],e[0],e[1],e[2],self.type.iang)
					self.state = LEVER_ON
					self.stick_obj.TimerFunc=None
		else:
...
def PlaceLever(name,type,position,orientation,scale):
	...
	lever.initialPosition = position
	lever.initialOrientation = orientation
